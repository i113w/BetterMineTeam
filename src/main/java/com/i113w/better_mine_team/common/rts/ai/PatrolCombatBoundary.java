...
public final class PatrolCombatBoundary {
    ...
    public static boolean hasActivePatrol(Mob mob) {
        return mob.getData(ModAttachments.UNIT_DATA).getPatrolTask().isEnabled();
    }
    ...
    static boolean contains(PatrolTask task, double x, double z, PatrolSettings settings) {
        ...
        return x >= min.getX() - paddingX
                && x <= max.getX() + 1.0D + paddingX
                && z >= min.getZ() - paddingZ
                && z <= max.getZ() + 1.0D + paddingZ;
    }
}
