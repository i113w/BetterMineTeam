...
public final class PatrolTargeting {
    ...
    public static boolean isTaskLocationLoaded(Level level, PatrolTask task) {
        if (task.getMode() == PatrolMode.POINT) return level.isLoaded(task.getCenter());
    ...
    public static List<BlockPos> createPerimeterRoute(PatrolTask task, PatrolSettings settings) {
        if (task.getMode() == PatrolMode.AREA) {
            return createAreaPerimeter(task.getMinCorner(), task.getMaxCorner(),
                    task.getCenter().getY(), settings.waypointSpacing());
        }
    ...
    public static Optional<PerimeterTarget> findReachablePerimeterTarget(
            PathfinderMob mob, List<BlockPos> route, int startIndex, int direction, PatrolSettings settings) {
    ...
    public static boolean isSafeStandPosition(PathfinderMob mob, BlockPos pos) {
        ...
        if (!belowState.isSolidRender()) return false;
