...
public final class PatrolTargeting {
    ...
    public static int areaWidth(BlockPos min, BlockPos max) {
        return max.getX() - min.getX() + 1;
    }

    public static int areaDepth(BlockPos min, BlockPos max) {
    ...
    public static boolean isTaskLocationLoaded(Level level, PatrolTask task) {
        if (task.getMode() == PatrolMode.POINT) {
            return level.isLoaded(task.getCenter());
        }
        return isAreaLoaded(level, task.getMinCorner(), task.getMaxCorner());
    }
    ...
    public static Optional<PerimeterTarget> findReachablePerimeterTarget(
            PathfinderMob mob,
            List<BlockPos> route,
            int startIndex,
            int direction,
        ...
        for (int i = 0; i < attempts; i++) {
            ...
            if (safe.isPresent() && canReach(mob, safe.get())) {
                return Optional.of(new PerimeterTarget(index, safe.get()));
            }
        }
        return Optional.empty();
    }

    public static Optional<BlockPos> findSafeStandPosition(PathfinderMob mob, BlockPos around,
                                                            PatrolSettings settings) {
        for (int y = around.getY() + settings.safeScanUp();
             y >= around.getY() - settings.safeScanDown();
             y--) {
            BlockPos candidate = new BlockPos(around.getX(), y, around.getZ());
            if (isSafeStandPosition(mob, candidate)) {
    ...
    public static boolean isSafeStandPosition(PathfinderMob mob, BlockPos pos) {
        ...
        if (!aboveState.getCollisionShape(level, above).isEmpty()) return false;
        if (aboveState.getFluidState().is(FluidTags.LAVA)) return false;

        BlockPos below = pos.below();
        BlockState belowState = level.getBlockState(below);
        if (!belowState.isSolidRender(level, below)) return false;
    ...
    private static List<BlockPos> createPointPerimeter(BlockPos center, int radius, PatrolSettings settings) {
        ...
        for (int i = 0; i < waypointCount; i++) {
            ...
            route.add(new BlockPos(x, center.getY(), z));
        }
        return List.copyOf(route);
    }

    private static void appendSegment(List<BlockPos> route, int startX, int startZ,
                                      int endX, int endZ, int y, int spacing) {
        ...
        for (int distance = 0; distance < length; distance += spacing) {
            addDistinct(route, new BlockPos(
