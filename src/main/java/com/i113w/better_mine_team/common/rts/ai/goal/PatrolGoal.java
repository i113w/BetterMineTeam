...
public class PatrolGoal extends Goal implements TeamGoal {
    ...
    @Override
    public boolean canContinueToUse() { return canPatrolNow(); }
    ...
    public void tick() {
        ...
        if (--repathTimer <= 0 || mob.getNavigation().isDone()) {
            ...
            if (!moving) handlePathFailure(task);
        }
    }
    ...
    private boolean isTaskStillValid(PatrolTask task, PatrolSettings settings) {
        if (!mob.level().dimension().identifier().toString().equals(task.getDimensionId())) return false;
        ...
        if (owner != null && mob instanceof TamableAnimal tamable) {
            var ownerReference = tamable.getOwnerReference();
            ownerMatches = ownerReference != null && owner.equals(ownerReference.getUUID());
        }
