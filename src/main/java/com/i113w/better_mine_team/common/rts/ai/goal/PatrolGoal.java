...
public class PatrolGoal extends Goal implements TeamGoal {
    ...
    public void tick() {
        ...
        if (targetPos == null) { chooseNextTarget(task, settings); return; }
        ...
        if (--repathTimer <= 0 || mob.getNavigation().isDone()) {
            ...
            boolean moving = mob.getNavigation().moveTo(targetPos.getX() + 0.5D, targetPos.getY(),
                    targetPos.getZ() + 0.5D, settings.movementSpeed());
            if (!moving) handlePathFailure(task, settings);
        }
    }
    ...
    private boolean canPatrolNow() {
        ...
        if (!settings.enabled() || !isTaskStillValid(task, settings)) {
            clearInvalidTask();
            return false;
        }
    ...
    private boolean isTaskStillValid(PatrolTask task, PatrolSettings settings) {
        ...
        if (task.getMode() == PatrolMode.AREA
                && !PatrolTargeting.isAreaSizeValid(task.getMinCorner(), task.getMaxCorner(), settings)) return false;
        PlayerTeam mobTeam = TeamManager.getTeam(mob);
        boolean teamMatches = !task.getTeamName().isBlank() && mobTeam != null
                && task.getTeamName().equals(mobTeam.getName());
        UUID owner = task.getOwnerUuid();
        boolean ownerMatches = owner != null && mob instanceof TamableAnimal tamable
                && owner.equals(tamable.getOwnerUUID());
    ...
    private boolean hasCombatPriority() {
        ...
        if (currentTarget != null && currentTarget.isAlive() && !TeamManager.isAlly(mob, currentTarget)) return true;
        return TeamManager.getBestThreat(TeamManager.getTeam(mob), mob) != null;
    }

    private void chooseNextTarget(PatrolTask task, PatrolSettings settings) {
        ensurePerimeterRoute(task, settings);
        if (perimeterRoute.isEmpty()) { waitTicks = settings.routeRetryDelayTicks(); return; }
        ...
        Optional<PatrolTargeting.PerimeterTarget> next = PatrolTargeting.findReachablePerimeterTarget(
                mob, perimeterRoute, startIndex, patrolDirection, settings);
        if (next.isPresent()) {
            PatrolTargeting.PerimeterTarget waypoint = next.get();
            targetPos = waypoint.position();
            nextWaypointIndex = Math.floorMod(waypoint.index() + patrolDirection, perimeterRoute.size());
            repathTimer = 0;
            return;
        }
    ...
    private void handlePathFailure(PatrolTask task, PatrolSettings settings) {
        retryCount++;
        targetPos = null;
        if (retryCount <= settings.pathRetryLimit()) return;
        task.setLastValidPos(mob.blockPosition());
        data.setPatrolTask(task);
        mob.getNavigation().stop();
        retryCount = 0;
    ...
    private void ensurePerimeterRoute(PatrolTask task, PatrolSettings settings) {
        if (hasSameRoute(activeTask, task)
                && activeSettingsRevision == settings.revision()) return;
        activeTask = task;
        ...
        nextWaypointIndex = PatrolTargeting.findNearestPerimeterIndex(perimeterRoute, mob.blockPosition());
        targetPos = null;
        repathTimer = 0;
        retryCount = 0;
        waitTicks = 0;
    ...
    private static boolean hasSameRoute(PatrolTask previous, PatrolTask current) {
        if (previous == null || current == null || previous.getMode() != current.getMode()) return false;
        if (!previous.getCenter().equals(current.getCenter())) return false;
        if (previous.getMode() == PatrolMode.POINT) return previous.getRadius() == current.getRadius();
        return previous.getMinCorner().equals(current.getMinCorner())
                && previous.getMaxCorner().equals(current.getMaxCorner());
    }
}
