...
public final class BMTClientConfig {
    ...
    static {
        ...
        markerMinHalfSize = builder.comment("Minimum half-size of the marker above a patrolling mob.")
                .defineInRange("markerMinHalfSize", 0.25D, 0.05D, 4.0D);
        markerWidthMultiplier = builder.comment("Mob width multiplier used to size the Patrol marker.")
                .defineInRange("markerWidthMultiplier", 0.35D, 0.0D, 2.0D);
        markerVerticalOffset = builder.comment("Vertical offset above the mob bounding box for the Patrol marker.")
                .defineInRange("markerVerticalOffset", 0.35D, -2.0D, 8.0D);
        markerHeight = builder.comment("Height of the Patrol marker box.")
                .defineInRange("markerHeight", 0.45D, 0.01D, 8.0D);
        groundBoxVerticalOffset = builder.comment("Vertical offset applied to Patrol ground boxes.")
                .defineInRange("groundBoxVerticalOffset", 0.05D, -1.0D, 2.0D);
        groundBoxHeight = builder.comment("Height of Patrol ground boxes.")
    ...
    private static ModConfigSpec.ConfigValue<String> defineColor(ModConfigSpec.Builder builder,
        ...
        return builder.comment("RGBA color in #RRGGBBAA format.")
                .define(name, defaultValue,
                        value -> value instanceof String text && RGBA_HEX.matcher(text).matches());
    }
