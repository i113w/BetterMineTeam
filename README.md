...
*   **Ender Dragon Riding**:
    ...
    *   Ride it by right-clicking with an empty hand. Use `Space` to accelerate/ascend and `Shift` to decelerate/descend.
*   **Configuration**:
    *   Added `.cfg` configuration options for almost all features.
...
*   **Scoreboard Teams**: The teams in this mod are based on the vanilla **Scoreboard Team** system. Therefore, it may not work when loaded alongside other mods that significantly modify scoreboard teams. Generally, vanilla-compliant scoreboard mods should work fine (except for the team switching button in the inventory). If issues arise, please submit an Issue.
